...
def browse_dest_path():
    ...
    if folder_selected == '': # 사용자가 취소를 누를 때. is None은 적용 안 됨.
        return
...
def merge_image():
    try:
        ...
        if img_width == "원본유지":
            img_width = -1 # -1일 때는 원본 기준으로 이미지 통합
        else:
        ...
        if img_width > -1: # widths 값이 변경되었을 때
        ...
        # 큰 스케치북 준비
        if img_space > 0: # 이미지 간격이 있다면
            total_height += (img_space * (len(images) - 1)) # 전체 높이를 간격만큼 더해줌
        ...
        for idx, img in enumerate(images): # images를 순회하면서 idx와 img를 갖고옴
            ...
            if img_width > -1: # 원본유지가 아닐 때
                img = img.resize(image_sizes[idx]) # 이미지 width, height를 갖고있는 image_sizes에서 index로 찾아서 사이즈 수정
        ...
        # 포맷 옵션 처리
        file_name = "merge_photo." + img_format # 파일이름+확장자명
        dest_path = os.path.join(txt_dest_path.get(), file_name) # txt_dest_path 경로에 merge_photo를 더한 것이 최종경로
